...
void mitk::DoseImageVtkMapper2D::GenerateDataForRenderer(mitk::BaseRenderer* renderer)
{
	...
	datanode->GetBoolProperty("in plane resample extent by geometry", inPlaneResampleExtentByGeometry,
	                          renderer);
	...
	if (input->GetPixelType().GetNumberOfComponents() ==
	    1)  // for now only single component are allowed
	{
	...
	if (thickSlicesMode > 0)
	{
		...
		input->GetTimeGeometry()->GetGeometryForTimeStep(this->GetTimestep())->WorldToIndex(normal,
		        normInIndex);
	...
	// calculate minimum bounding rect of IMAGE in texture
	{
		...
		mitk::PlaneClipping::CalculateClippedPlaneBounds(input->GetGeometry(), planeGeometry,
		        textureClippingBounds);

		textureClippingBounds[0] = static_cast< int >(textureClippingBounds[0] /
		                           localStorage->m_mmPerPixel[0] + 0.5);
		textureClippingBounds[1] = static_cast< int >(textureClippingBounds[1] /
		                           localStorage->m_mmPerPixel[0] + 0.5);
		textureClippingBounds[2] = static_cast< int >(textureClippingBounds[2] /
		                           localStorage->m_mmPerPixel[1] + 0.5);
		textureClippingBounds[3] = static_cast< int >(textureClippingBounds[3] /
		                           localStorage->m_mmPerPixel[1] + 0.5);
	...
	if (datanode->GetIntProperty("Image.Displayed Component", displayedComponent, renderer)
	    && numberOfComponents > 1)
	{
		...
		localStorage->m_LevelWindowFilter->SetInputConnection(
		    localStorage->m_VectorComponentExtractor->GetOutputPort(0));
	}
	else
	{
	...
	vtkActor* contourShadowActor = dynamic_cast<vtkActor*>
	                               (localStorage->m_Actors->GetParts()->GetItemAsObject(0));
...
void mitk::DoseImageVtkMapper2D::ApplyColor(mitk::BaseRenderer* renderer)
{
	...
	if (hover && !selected)
	{
		mitk::ColorProperty::Pointer colorprop = dynamic_cast<mitk::ColorProperty*>
	...
	dynamic_cast<vtkActor*>(localStorage->m_Actors->GetParts()->GetItemAsObject(
	                            0))->GetProperty()->SetColor(rgbConv);
...
void mitk::DoseImageVtkMapper2D::ApplyOpacity(mitk::BaseRenderer* renderer)
{
	...
	{
		dynamic_cast<vtkActor*>(localStorage->m_Actors->GetParts()->GetItemAsObject(
		                            0))->GetProperty()->SetOpacity(opacity);
	}
}
...
void mitk::DoseImageVtkMapper2D::ApplyRenderingMode(mitk::BaseRenderer* renderer)
{
	...
	else
	{
		...
		mitk::RenderingModeProperty::Pointer mode = dynamic_cast<mitk::RenderingModeProperty*>
		        (this->GetDataNode()->GetProperty("Image Rendering.Mode", renderer));
...
void mitk::DoseImageVtkMapper2D::ApplyLookuptable(mitk::BaseRenderer* renderer)
{
	...
	mitk::LookupTableProperty::Pointer lookupTableProp = dynamic_cast<mitk::LookupTableProperty*>
...
void mitk::DoseImageVtkMapper2D::ApplyColorTransferFunction(mitk::BaseRenderer* renderer)
{
	mitk::TransferFunctionProperty::Pointer transferFunctionProp =
	    dynamic_cast<mitk::TransferFunctionProperty*>
	...
	localStorage->m_LevelWindowFilter->SetLookupTable(
	    transferFunctionProp->GetValue()->GetColorTransferFunction());
}
...
void mitk::DoseImageVtkMapper2D::Update(mitk::BaseRenderer* renderer)
{
	...
	if ((localStorage->m_LastUpdateTime < node->GetMTime())  //was the node modified?
	    ...
	    || (localStorage->m_LastUpdateTime <
	        renderer->GetCurrentWorldGeometry2DUpdateTime()) //was the geometry modified?
        ...
        mitk::BaseRenderer* renderer, bool overwrite)
{
	...
	node->AddProperty("outline binary shadow color", ColorProperty::New(0.0, 0.0, 0.0), renderer,
	                  overwrite);
	node->AddProperty("outline shadow width", mitk::FloatProperty::New(1.5), renderer, overwrite);

	if (image->IsRotated())
	{
		node->AddProperty("reslice interpolation",
		                  mitk::VtkResliceInterpolationProperty::New(VTK_RESLICE_CUBIC));
	}
	else
	{
	...
	if (node->GetStringProperty("dicom.pixel.PhotometricInterpretation", photometricInterpretation))
	{
		...
		if (photometricInterpretation.find("MONOCHROME1") !=
		    std::string::npos)   // meaning: display MINIMUM pixels as WHITE
		{
	...
	if (isBinaryImage)
	{
		...
		node->AddProperty("binaryimage.selectedcolor", ColorProperty::New(1.0, 0.0, 0.0), renderer,
		                  overwrite);
		node->AddProperty("binaryimage.selectedannotationcolor", ColorProperty::New(1.0, 0.0, 0.0),
		                  renderer, overwrite);
		node->AddProperty("binaryimage.hoveringcolor", ColorProperty::New(1.0, 0.0, 0.0), renderer,
		                  overwrite);
		node->AddProperty("binaryimage.hoveringannotationcolor", ColorProperty::New(1.0, 0.0, 0.0),
		                  renderer, overwrite);
	...
	if (image.IsNotNull() && image->IsInitialized())
	{
		if ((overwrite) || (node->GetProperty("levelwindow", renderer) == NULL))
		{
			    ...
			    && image->GetPropertyList()->GetStringProperty("dicom.voilut.WindowWidth", sWindow))
			{
				...
				if (image->GetPropertyList()->GetStringProperty("dicom.series.SmallestPixelValueInSeries",
				        sSmallestPixelValueInSeries)
				    && image->GetPropertyList()->GetStringProperty("dicom.series.LargestPixelValueInSeries",
				...
				{
					contrast.SetAuto(static_cast<mitk::Image*>(node->GetData()), false,
					                 true);   // we need this as a fallback
				}
...
mitk::DoseImageVtkMapper2D::LocalStorage* mitk::DoseImageVtkMapper2D::GetLocalStorage(
...
vtkSmartPointer<vtkPolyData> mitk::DoseImageVtkMapper2D::CreateOutlinePolyData(
    mitk::BaseRenderer* renderer)
{
	vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New(); //the points to draw
	vtkSmartPointer<vtkCellArray> lines =
	    vtkSmartPointer<vtkCellArray>::New(); //the lines to connect the points
	...
	mitk::IsoDoseLevelSetProperty::Pointer propIsoSet = dynamic_cast<mitk::IsoDoseLevelSetProperty* >
	        (GetDataNode()->GetProperty(mitk::Constants::DOSE_ISO_LEVELS_PROPERTY_NAME.c_str()));
	mitk::IsoDoseLevelSet::Pointer isoDoseLevelSet = propIsoSet->GetValue();

	if (isoDoseLevelSet.IsNotNull())
	{
		for (mitk::IsoDoseLevelSet::ConstIterator doseIT = isoDoseLevelSet->Begin();
		     doseIT != isoDoseLevelSet->End(); ++doseIT)
		{
	...
	mitk::IsoDoseLevelVectorProperty::Pointer propfreeIsoVec =
	    dynamic_cast<mitk::IsoDoseLevelVectorProperty* >(GetDataNode()->GetProperty(
	                mitk::Constants::DOSE_FREE_ISO_VALUES_PROPERTY_NAME.c_str()));
	...
	if (frereIsoDoseLevelVec.IsNotNull())
	{
		for (mitk::IsoDoseLevelVector::ConstIterator freeDoseIT = frereIsoDoseLevelVec->Begin();
		     freeDoseIT != frereIsoDoseLevelVec->End(); ++freeDoseIT)
		{
...
void mitk::DoseImageVtkMapper2D::CreateLevelOutline(mitk::BaseRenderer* renderer,
        const mitk::IsoDoseLevel* level, float pref, vtkSmartPointer<vtkPoints> points,
        vtkSmartPointer<vtkCellArray> lines,  vtkSmartPointer<vtkUnsignedCharArray> colors)
{
	LocalStorage* localStorage = this->GetLocalStorage(renderer);
	...
	while (y <= yMax)
	{
		...
		if ((currentPixel) && (*currentPixel >= doseValue))
		{
			...
			if (y > yMin && *(currentPixel - line) < doseValue)
			{
				...
				//add the 2 points
				vtkIdType p1 = points->InsertNextPoint(x * localStorage->m_mmPerPixel[0],
				                                       y * localStorage->m_mmPerPixel[1], depth);
				vtkIdType p2 = points->InsertNextPoint((x + 1) * localStorage->m_mmPerPixel[0],
				                                       y * localStorage->m_mmPerPixel[1], depth);
			...
			if (y < yMax && *(currentPixel + line) < doseValue)
			{
				...
				vtkIdType p1 = points->InsertNextPoint(x * localStorage->m_mmPerPixel[0],
				                                       (y + 1) * localStorage->m_mmPerPixel[1], depth);
				vtkIdType p2 = points->InsertNextPoint((x + 1) * localStorage->m_mmPerPixel[0],
				                                       (y + 1) * localStorage->m_mmPerPixel[1], depth);
				lines->InsertNextCell(2);
				lines->InsertCellPoint(p1);
			...
			if ((x > xMin || y > yMin) && *(currentPixel - 1) < doseValue)
			{
				...
				vtkIdType p1 = points->InsertNextPoint(x * localStorage->m_mmPerPixel[0],
				                                       y * localStorage->m_mmPerPixel[1], depth);
				vtkIdType p2 = points->InsertNextPoint(x * localStorage->m_mmPerPixel[0],
				                                       (y + 1) * localStorage->m_mmPerPixel[1], depth);
				lines->InsertNextCell(2);
				lines->InsertCellPoint(p1);
				lines->InsertCellPoint(p2);
			...
			if ((y < yMax || (x < xMax)) && *(currentPixel + 1) < doseValue)
			{
				...
				vtkIdType p1 = points->InsertNextPoint((x + 1) * localStorage->m_mmPerPixel[0],
				                                       y * localStorage->m_mmPerPixel[1], depth);
				vtkIdType p2 = points->InsertNextPoint((x + 1) * localStorage->m_mmPerPixel[0],
				                                       (y + 1) * localStorage->m_mmPerPixel[1], depth);
				lines->InsertNextCell(2);
...
void mitk::DoseImageVtkMapper2D::TransformActor(mitk::BaseRenderer* renderer)
{
	...
	localStorage->m_Actor->SetPosition(-0.5 * localStorage->m_mmPerPixel[0],
	                                   -0.5 * localStorage->m_mmPerPixel[1], 0.0);

	if (localStorage->m_Actors->GetNumberOfPaths() > 1)
	{
		vtkActor* secondaryActor = dynamic_cast<vtkActor*>
		                           (localStorage->m_Actors->GetParts()->GetItemAsObject(0));
		secondaryActor->SetUserTransform(trans);
		secondaryActor->SetPosition(-0.5 * localStorage->m_mmPerPixel[0],
		                            -0.5 * localStorage->m_mmPerPixel[1], 0.0);
	}
}

bool mitk::DoseImageVtkMapper2D::RenderingGeometryIntersectsImage(const Geometry2D*
        renderingGeometry, SlicedGeometry3D* imageGeometry)
{
